// [library:3d] OpenGL ES 3 / WebGL2 render backend. Forked from the desktop OpenGL 3.3
// backend: same D3DTLVERTEX rasterizer, offscreen scene FBO and letterbox-blit present, but
// with an ES 3.0 context, GLSL ES 3.00 shaders, and no GL_BGRA vertex format (WebGL2 lacks
// it, so the D3DCOLOR bytes are swizzled in the shader instead).
...
layout(location = 1) in vec4 aColor;    // D3DCOLOR diffuse (BGRA byte order)
layout(location = 2) in vec4 aSpecular; // D3DCOLOR specular (BGRA byte order)
...
void main()
{
	...
	// WebGL2/GLES has no GL_BGRA vertex format, so the D3DCOLOR bytes arrive in RGBA
	// component order; swizzle .bgra to recover the true RGBA color (the desktop GL3
	// backend gets this reorder for free from the GL_BGRA attribute).
	vColor = aColor.bgra;
...
static const char* g_fragmentShader = R"GLSL(#version 300 es
precision highp float;
precision highp int;
...
class MiniwinGles3Backend : public MiniwinRenderBackend {
public:
...
private:
	...
	void EnsureSceneFbo(int p_width, int p_height);
	void UpdateViewport();
	GLint m_uAlphaRef = -1;
...
{
	// A GL/GLES context cannot be probed before a window exists, so report available and
	// let Create fail cleanly if the platform has no ES driver (only reachable when GLES3
	// is explicitly requested — it is never the desktop default). On the web it is always
	// available (WebGL2).
	return true;
}
...
{
	MiniwinGles3Backend* backend = new MiniwinGles3Backend();
...
GLuint MiniwinGles3Backend::CompileShader(GLenum p_type, const char* p_source)
{
...
bool MiniwinGles3Backend::Init(SDL_Window* p_window)
{
	m_window = p_window;
	...
	if (!gl.LoadAll()) {
		SDL_LogError(LOG_CATEGORY_MINIWIN, "Failed to load OpenGL ES 3 functions");
		return false;
	}
	...
	// No GL_BGRA in GLES/WebGL2: fetch the D3DCOLOR bytes as normalized RGBA and swizzle
	// to real RGBA in the vertex shader.
	gl.glVertexAttribPointer(1, 4, GL_UNSIGNED_BYTE, GL_TRUE, stride, (void*) offsetof(D3DTLVERTEX, color));
	gl.glEnableVertexAttribArray(1);
	gl.glVertexAttribPointer(2, 4, GL_UNSIGNED_BYTE, GL_TRUE, stride, (void*) offsetof(D3DTLVERTEX, specular));
...
// Creates (or resizes) the offscreen scene target and leaves it bound as the draw
// framebuffer. All rendering goes through it: DirectDraw's backbuffer retains its
// contents across flips (the loading screen presents a bitmap once and then draws
// progress dots over it for minutes), while a GL default framebuffer is undefined
// after every swap. The target uses the logical resolution in
// MINIWIN_RESOLUTION_ORIGINAL mode and the drawable size otherwise.
void MiniwinGles3Backend::EnsureSceneFbo(int p_width, int p_height)
{
	if (m_sceneFbo && m_sceneFboW == p_width && m_sceneFboH == p_height) {
	...
	gl.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, p_width, p_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
	gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);

	gl.glGenRenderbuffers(1, &m_sceneDepth);
	gl.glBindRenderbuffer(GL_RENDERBUFFER, m_sceneDepth);
	gl.glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, p_width, p_height);
...
void MiniwinGles3Backend::UpdateViewport()
{
	int dw = 0;
	int dh = 0;
	...
	if (MiniwinGetRenderResolution() == MINIWIN_RESOLUTION_ORIGINAL) {
		EnsureSceneFbo(m_width, m_height);
	}
	else {
		EnsureSceneFbo(dw, dh);
	}

	gl.glViewport(0, 0, m_sceneFboW, m_sceneFboH);
}

void MiniwinGles3Backend::Resize(int p_width, int p_height)
{
	m_width = p_width;
	m_height = p_height;
	gl.glUseProgram(m_program);
...
Uint32 MiniwinGles3Backend::CreateTexture(int p_width, int p_height, const void* p_rgba, bool p_mipmaps)
{
	GLuint id = 0;
...
static Uint32 g_miniwinStatTexUploads;
static Uint64 g_miniwinStatTexUploadBytes;

void MiniwinGles3Backend::UpdateTexture(Uint32 p_id, int p_width, int p_height, const void* p_rgba, bool p_mipmaps)
{
...
void MiniwinGles3Backend::DestroyTexture(Uint32 p_id)
{
	GLuint id = p_id;
	gl.glDeleteTextures(1, &id);
}

void MiniwinGles3Backend::BeginScene()
{
}

void MiniwinGles3Backend::Clear(const SDL_Rect* p_rect, float p_r, float p_g, float p_b, bool p_color, bool p_depth)
{
	// Scissor so partial clears stay inside their rect. Everything renders into the
	// scene target, whose coordinates map from the logical resolution.
	float sx = (float) m_sceneFboW / (float) m_width;
	float sy = (float) m_sceneFboH / (float) m_height;
	bool scissor = true;
	...
	if (p_rect && (p_rect->x != 0 || p_rect->y != 0 || p_rect->w != m_width || p_rect->h != m_height)) {
		...
		int y = m_sceneFboH - (int) ((p_rect->y + p_rect->h) * sy);
		gl.glScissor(x, y, w, h);
	}
	else {
		gl.glScissor(0, 0, m_sceneFboW, m_sceneFboH);
	}
...
GLenum MiniwinGles3Backend::BlendFactor(D3DBLEND p_blend)
{
	switch (p_blend) {
	case D3DBLEND_ZERO:
		return GL_ZERO;
...
GLenum MiniwinGles3Backend::CompareFunc(D3DCMPFUNC p_func)
{
	switch (p_func) {
	case D3DCMP_NEVER:
		return GL_NEVER;
...
void MiniwinGles3Backend::ApplyState(const MiniwinRasterState& p_state)
{
	if (p_state.zEnable) {
		gl.glEnable(GL_DEPTH_TEST);
...
void MiniwinGles3Backend::DrawTriangles(
	const MiniwinRasterState& p_state,
	const D3DTLVERTEX* p_vertices,
	Uint32 p_vertexCount,
	const Uint16* p_indices,
...
)
{
	...
	if (traceEnv && m_frameCounter == (Uint64) SDL_atoi(traceEnv)) {
		...
		int px = (int) (cx * m_sceneFboW / (float) m_width);
		int py = m_sceneFboH - 1 - (int) (cy * m_sceneFboH / (float) m_height);
...
void MiniwinGles3Backend::EndScene()
{
}

void MiniwinGles3Backend::HandleFrameDump()
{
...
void MiniwinGles3Backend::Present()
{
...
void MiniwinGles3Backend::DrawOverlay(
	const D3DTLVERTEX* p_vertices,
	Uint32 p_vertexCount,
	const Uint16* p_indices,
	Uint32 p_indexCount
...
SDL_Surface* MiniwinGles3Backend::ReadBackbuffer()
{
	int dw = m_sceneFboW;
	int dh = m_sceneFboH;
	int rx = 0;
